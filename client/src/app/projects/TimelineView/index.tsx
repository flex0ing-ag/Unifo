...
const TimelinePage = ({ id, setIsModalNewTaskOpen }: Props) => {
  const isDarkMode = useAppSelector((state) => state.global.isDarkMode);
  ...
  const items = useMemo(() => {
    ...
    return tasks
      ...
      .map((task, index) => ({
        ...
        itemProps: {
          style: {
            background: isDarkMode ? "#334155" : "#60a5fa", // Dark: slate, Light: blue-400
            color: "white",
            borderRadius: "6px",
            padding: "4px 8px",
            fontWeight: "500",
  ...
  if (error || !tasks)
    return <div className="p-4 text-red-600">Error loading tasks.</div>;
